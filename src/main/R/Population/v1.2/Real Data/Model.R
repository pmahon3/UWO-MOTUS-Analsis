...
modelCode <- nimbleCode(
  {
    ...
    for (obs in 1:nObs){
        ...
        period <- step(t - delta[bird, day, 1]) + step(t - delta[bird, day, 2] - delta.prime[bird] * last) +  1
        
        sig[obs] ~ dnorm( muY[bird, day, period], tau[bird, day, period])
    }
    
    ## Period effects
    for(p in 1:3){
      for (bird in 1:nBirds){
        for (day in 1:nDays[bird]){
    ...
    for(p in 1:3){
      ## Mean model
      muMuY[p] ~ dnorm(etaMuMuY[p], 1 / thetaMuMuY[p]^2 )
